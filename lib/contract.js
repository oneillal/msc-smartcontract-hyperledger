...
//     // NEW CODE
//     //const templateDir = path.resolve(__dirname, '..', 'contract');
//     //
...
async function loadTemplateFromDisk(ctx, contractText, initClauses) {
    ...
    if(clauses.length > 0) {
        ...
        for( let n=0; n < clauses.length; n++ ) {
            ...
            const templateDir = path.resolve(__dirname, '..', 'peer2peerlibrary');
 ...
 * @param {*} ctx HLF context
 * @param {*} name the name of a clause present within the markdown text
 * @returns {Promise<string>} the markdown string
 */
async function ensureInitialized(ctx, name) {
    const markdown = await ctx.stub.getState(`Markdown-${name}`);
...
class Peer2PeerLibrary extends Contract {
     ...
     */
    async init(ctx, contractText, name) {

        await ensureNotInitialized(ctx, name);
        ...
        await ctx.stub.putState(`Markdown-${name}`, Buffer.from(contractText));
     ...
     */
    async invoke(ctx, requestText, name) {

        await ensureInitialized(ctx, name);
    ...
    async queryState(ctx, name) {
        const stateAsBytes = await ctx.stub.getState(`State-${name}`); // get the state from chaincode
        if (!stateAsBytes || stateAsBytes.length === 0) {
            throw new Error(`State-${name} state does not exist`);
        }
    ...
    async queryData(ctx, name) {
        const dataAsBytes = await ctx.stub.getState(`Data-${name}`); // get the data from chaincode state
