...
async function loadTemplateFromDisk(ctx, contractText, initClauses) {
    ...
    if(clauses.length > 0) {
        ...
        for( let n=0; n < clauses.length; n++ ) {
            ...
            // load from cta archive
            // const templateArchive = path.resolve(__dirname, '..', 'latereturnandpenalty_0.11.cta');
            // const buffer = fs.readFileSync(templateArchive);
